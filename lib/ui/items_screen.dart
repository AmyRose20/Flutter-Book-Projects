...
/* Each time we'll get to this screen, it will be because we have
selected a 'ShoppingList' object. We will never need to call this
 screen independently. So, it makes sense that when we create the
 'ItemScreen' widget, we expect a 'ShoppingList' to be passed. */
...
class _ItemsScreenState extends State<ItemsScreen> {
  ...
  Future showData(int idList) async {
    await helper.openDb();
    /* The result of the 'getItems()' method will be placed in the 'items'
    property. */
    items = await helper.getItems(idList);
    /* NB: 'setState()' is called to update the 'State' of the 'items' property,
    so that the UI will be redrawn. */
    setState(() {
      items = items;
    });
  }
